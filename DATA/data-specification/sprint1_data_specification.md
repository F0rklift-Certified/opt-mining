...
**Version:** 1.9
**Date:** 2026-09-12 (baseline frozen 2026-08-27)
**Status:** FROZEN — Sprint 1 baseline (v1.1–v1.9 amendments via §8)
...
| **Known limitations** | (1) Inherits every §4.8/§4.7/§4.5 limitation and coverage gap — the eligible population is the New-England-REZ raster-coverage window, so the checks judge ~1,233 scored cells rather than a statewide candidate set. (2) It is a plausibility screen at the ~5 km analysis-cell resolution, **not** a formal accuracy assessment or a site approval; a farm-to-cell containment is only as precise as the cell. (3) The feature spot-checks surface values for a **human** reviewer to verify against source data — the discrepancy fields are intentionally left blank and are not auto-filled. (4) It reports anomalies and logs Sprint2_Issues but **never** re-scores, re-ranks, re-weights or otherwise adjusts the model to make a check pass |

---

### 4.10 Site Explanations (Derived — S2-06a)

Added via the §8 "Adding a New Dataset" process (v1.9). **Documented gap:** the §4.7 Baseline Suitability Score retains a per-criterion contribution (`contrib_{feature}`) for every eligible cell, but there was no human-readable narrative explaining *why* a cell scored as it did; S2-06b (excluded-cell explanations + proxy/data-quality caveats), S2-08 (`get_site_detail`) and the Sprint 3 site-detail view (S3-05) all need one structured explanation per site. No such dataset existed. **Integration assessment:** consumes only the §4.7 Baseline Suitability Score (the persisted `contrib_{feature}`, score and rank) and — read only, to recompute the normalised values the qualitative bands need — the §4.5 Integrated Feature Table, with the §4.7 criteria weights file. Same grid (§3), same storage CRS (EPSG:4326); no reprojection, no re-scoring, no re-ranking, no new external source enters the platform. The normalised values are recomputed with the **same** normaliser the scoring stage uses (`pipeline/scoring/normalise.py`) and a reconciliation guard asserts the recomputed contributions reproduce the persisted §4.7 values within 1e-9 before any explanation is written, so the bands are provably derived from the values that produced the score.

| Field | Value |
|-------|-------|
| **Dataset name** | Site Explanations — deterministic, template/rule-based explanation of every eligible cell (positive factors and weaknesses) |
| **Source** | **DERIVED** by `pipeline/explanation/` (the `explanation` stage, S2-06a) from the §4.7 Baseline Suitability Score (`DATA/scoring/optmining_suitability-score_2026_nsw.gpkg`), the §4.5 Integrated Feature Table (read only, to recompute normalisation bounds) and two user-input config files — the explanation templates `pipeline/explanation/explanation_templates.yaml` and the criteria weights `pipeline/scoring/scoring_weights.yaml` (neither is a dataset) |
...
| **Variable(s) — Explanation_Structure fields (eligible path, frozen here)** | `cell_id` (str, grid identifier, reused byte-for-byte from §4.7); `eligible` (bool, `true` for every record on this path); `headline` (str, one screening-level headline); `positive_factors` (array of str, the strongest factors ranked by §4.7 contribution magnitude, each with a qualitative band, at most 3); `weaknesses` (array of str, the criteria the cell scores poorly on, worst first, at most 2). **S2-06b extends this structure in place** — adding `exclusion_reasons`, `proxy_caveats` and `data_quality_notes` for the excluded-cell path — and must not rename or remove any field above |
...
| **Method** | **Deterministic template/rule engine — NO language model.** For each eligible cell: positive factors are the criteria contributing the most to the §4.7 score, ranked by the persisted `contrib_{feature}` (never recomputed), among criteria the cell scores favourably on; weaknesses are the criteria with the lowest normalised value; a qualitative band derived from the recomputed normalised value is appended to each factor phrase. The phrasing (per-criterion positive/weakness phrases, band labels and thresholds, the headline) is **user input** loaded at runtime from `explanation_templates.yaml` (or `--explanation-templates PATH`); no phrase literal appears in `pipeline/explanation/` source. Screening-level language throughout ("higher-ranked candidate under the selected assumptions and criteria", never "best site"); non-screening superlatives are rejected at config load and asserted absent by validation |
| **Determinism** | The same §4.7 Scored_Table and the same templates always produce byte-identical explanations. There is exactly one normaliser (the shared `pipeline/scoring/normalise.py`); the recomputed contributions are reconciled to the persisted §4.7 values within 1e-9 before anything is written |
| **Vintage token** | `2026` — tracks the §4.7 Scored_Table it explains, so the products are visibly the same generation of the data |
| **Role in model** | The explanation layer of the Constitution's "a recommendation the user cannot interrogate is not a recommendation" mandate. Sole consumer of §4.7 for narrative purposes; the eligible Explanation_Structure is the **frozen contract** S2-06b extends and S2-08 `get_site_detail` / S3-05 consume |
...
| **Known limitations** | (1) Inherits every §4.7 (and thus §4.5/§4.6) limitation and coverage gap — explanations exist only for the ~1,233 eligible cells of the New-England-REZ raster-coverage window. (2) This ticket (S2-06a) covers the **eligible-cell path only**; excluded-cell explanations and the proxy/data-quality caveat rules are **S2-06b**, which extends this schema. Until then the `demand_proxy` constant-criterion caveat (§4.7) is not yet surfaced in the narrative. (3) It is a screening explanation at the ~5 km analysis-cell resolution — it reports which criteria drove a cell's relative rank, not a site approval, a cost or an engineering-grade assessment |
...
| Site Explanations (§4.10) | Baseline Suitability Score (§4.7) + Integrated Feature Table (§4.5, read only to recompute norms) + explanation templates (`pipeline/explanation/explanation_templates.yaml`) + criteria weights (`pipeline/scoring/scoring_weights.yaml`) → `explanation` (S2-06a) | One deterministic Explanation_Structure per eligible cell (`cell_id`, `eligible`, `headline`, `positive_factors`, `weaknesses`) — `DATA/explanation/optmining_site-explanations_2026_nsw.json` + `.csv` | All criteria — deterministic eligible-cell explanation; frozen contract extended by S2-06b and consumed by S2-08 `get_site_detail` / S3-05 |
