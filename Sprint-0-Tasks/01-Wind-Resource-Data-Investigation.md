...
All 31 combinations were tested with HTTP HEAD by `pipeline/wind/probe.py`; full record in `DATA/wind-resource/metadata/layer_availability.md`. Sizes are for full Australian coverage.
...
**11 MB retrieved in total, against 3.16 GB if these five layers had been downloaded whole.** The CDN advertises `accept-ranges: bytes` and the rasters are internally tiled with overview pyramids, so `pipeline/wind/download.py` opens each raster through GDAL's `/vsicurl/` driver and reads only the pixel window it needs. No full-country raster was ever stored. This satisfies both the Constitution ("do not download or process more data than the analysis requires") and the Atlas's own access terms, which prohibit using the API for bulk downloads.
...
Generated by `pipeline/wind/inspect.py`; full records in `DATA/wind-resource/metadata/*_inspection.md`.
...
- [x] Reproducible retrieval, inspection and validation stages under `pipeline/wind/`
...
| Path | Contents |
|---|---|
| `pipeline/wind/gwa.py` | Shared endpoint resolution and GDAL settings |
| `pipeline/wind/probe.py` | Probes every variable/height combination for availability and size |
| `pipeline/wind/download.py` | Windowed `/vsicurl/` retrieval; writes the download manifest |
| `pipeline/wind/inspect.py` | Raster metadata and statistics; local files, remote headers, or decimated whole-raster summaries |
| `pipeline/wind/validate.py` | Samples the Atlas at known wind farm locations; crosscheck against prototype |
| `pipeline/wind/analyse.py` | Quantifies the 250 m → 5 km aggregation choice |
| `pipeline/wind/config.py` | GWA URLs, wind paths, aggregation constants |
...
# Individual stages
python -m pipeline --only wind.probe             # §4a availability table
python -m pipeline --only wind.download          # §5 sample clips
python -m pipeline --only wind.inspect           # §6 clip statistics
python -m pipeline --only wind.validate          # §9 validation + crosscheck
python -m pipeline --only wind.analyse           # §8 issue 2 evidence

# Or run the full wind domain at once
...
2. **`ingest_wind.load()` still raises `DataNotAvailableError` for real data.** The rasters are on disk and verified; only the loader is missing. This task's `pipeline/wind/download.py` and `pipeline/wind/inspect.py` provide the windowed-read and CRS-handling patterns it needs.
