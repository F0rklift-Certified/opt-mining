...
Derived terrain rasters (generated by `pipeline/geographic/derive.py`, stored as scaled int16): `srtm-gl3_slope-horn_90m_new-england-rez.tif` (8.5 MB), `srtm-gl1_slope-horn_30m_glen-innes.tif` (4.8 MB), `srtm-gl1_tri_30m_glen-innes.tif` (3.5 MB), all in `DATA/geographic/elevation/`.
...
Derived rasters (this task, `pipeline/geographic/derive.py`): Horn slope in degrees (int16 × 0.01 scale) and Riley TRI in metres (int16 × 0.1 scale), scale factors declared in the GeoTIFF band metadata.
...
| ABS service default SR | The ASGS service *source* SR is EPSG:3857 and queries return Web Mercator unless `outSR` is explicit | Med | Always pass `outSR` (done in `pipeline/common/geo.py`) | Yes |
...
**Slope derivation.** Horn's 3×3 method with latitude-corrected spacing, implemented in `pipeline/geographic/derive.py` with rasterio+numpy alone — no new dependencies. A pre-computed CSIRO slope grid exists (registration required) if a national run later wants to skip derivation. The aggregation statistic (mean/max/p90 per cell) changes exclusion shares by up to 74 percentage points at the same threshold; that choice is Task 5's, with the evidence table ready.
...
| Path | Contents |
|------|----------|
| `pipeline/common/geo.py` | Shared helpers: window constants, ArcGIS paged-GeoJSON queries (explicit `outSR`), atomic writes, GDAL remote-read env |
| `pipeline/geographic/probe.py` | Metadata-only probe of 21 sources → `source_register.md`/`.csv` |
| `pipeline/geographic/download.py` | ABS/CAPAD/Natural Earth samples + derived NEM regions + SRTM windowed clips + NLUM → manifest |
| `pipeline/geographic/inspect.py` | One inspection report per sample (11) |
| `pipeline/geographic/derive.py` | Horn slope + Riley TRI rasters; GL1-vs-GL3 and aggregation evidence → `slope_derivation.md` |
| `pipeline/geographic/validate.py` | NE-vs-ABS mask comparison, ground-truth checks incl. EPSG:3577 area proofs and wind-farm survival → `validation_geographic.md`, `landmask_assessment.md` |
| `pipeline/geographic/config.py` | ArcGIS endpoints, SRTM/NLUM URLs, geographic paths |
...
```bash
# Individual stages
python -m pipeline --only geographic.probe       # source register
python -m pipeline --only geographic.download    # vectors + rasters
python -m pipeline --only geographic.inspect     # sample inspection
python -m pipeline --only geographic.derive      # slope + TRI
python -m pipeline --only geographic.validate    # ground-truth checks + land mask

# Or run the full geographic domain at once
