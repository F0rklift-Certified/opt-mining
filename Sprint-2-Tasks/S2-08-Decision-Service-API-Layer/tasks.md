...
> **Status: COMPLETE (all tasks 1-9 delivered on branch `s2-08`).**
> The thin `pipeline/service/` layer is implemented and the OpenAPI contract is
> frozen. Delivered modules: `CONTRACT.md` (frozen contract), `models.py`,
> `runs.py`, `run_analysis.py`, `results.py` (get_ranked_results, get_site_detail,
> get_exclusions), `filters.py`, `scenarios.py` (compare_scenarios), `quality.py`
> (get_data_quality) and the FastAPI transport shell `app.py` (six endpoints +
> `/openapi.json`, `/docs`). All six correctness properties P1-P6 have property
> tests; contract/integration tests exercise every endpoint over the real engine.
> Full `tests/service/` suite: 133 passed, 1 skipped (the skip is
> environment-conditional -- the S2-02 validation sidecar is not materialised in
> the checkout, so `/data-quality` correctly returns the honest 503). See
> `pipeline/service/CONTRACT.md` for the frozen surface handed to S3-01a/S3-01b.
...
- [x] 1. Define and freeze the service contract
  - [x] 1.1 Author `pipeline/service/CONTRACT.md`
...
- [x] 2. Implement run_analysis
  - [x] 2.1 Implement `run_analysis` in `pipeline/service/run_analysis.py`
...
- [x] 3. Implement the read operations over materialised output
  - [x] 3.1 Implement `get_ranked_results` in `pipeline/service/results.py`
  ...
  - [x] 3.2 Implement `get_site_detail` in `pipeline/service/results.py`
  ...
  - [x] 3.3 Implement `get_exclusions` in `pipeline/service/results.py`
    - Return excluded cells with machine- and human-readable reasons from the Eligibility_Table (1.4)
    - _Requirements: 1.4_

  - [x] 3.4 Property test — one engine output across operations
...
- [x] 4. Implement the display filters
  - [x] 4.1 Implement top-N and minimum-score filters in `pipeline/service/filters.py`
  ...
  - [x] 4.2 Property test — filters do not re-score
  ...
  - [x] 4.3 Property test — empty-but-valid results
...
- [x] 5. Implement scenario comparison and data-quality surfacing
  - [x] 5.1 Implement `compare_scenarios` in `pipeline/service/scenarios.py`
  ...
  - [x] 5.2 Implement `get_data_quality` in `pipeline/service/quality.py`
...
- [x] 7. Implement honest-failure error handling
  ...
  - [x] 7.2 Property test — no recompute path
  ...
  - [x] 7.3 Property test — honest failure
...
- [x] 8. Contract and integration tests against the real engine
  - [x] 8.1 Write contract/integration tests
...
- [x] 9. Checkpoint — contract frozen for Sprint 3
