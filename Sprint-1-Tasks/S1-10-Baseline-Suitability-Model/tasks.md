...
- [ ] 1. Add scoring config and shipped default weights
  - [ ] 1.1 Create `pipeline/scoring/config.py` and the default `pipeline/scoring/scoring_weights.yaml`
...
- [ ] 2. Implement the weights loader and validator
  - [ ] 2.1 Implement `Criterion`/`WeightsConfig` dataclasses and `load_weights` in `pipeline/scoring/weights.py`
  ...
  - [ ]* 2.2 Write property test — weights come from configuration; invalid configs rejected
...
- [ ] 3. Implement the integrated-table loader with fail-fast validation
  - [ ] 3.1 Implement `load_integrated` in `pipeline/scoring/load.py`
  ...
  - [ ]* 3.2 Write unit tests for loader error conditions
...
- [ ] 4. Implement normalisation
  - [ ] 4.1 Implement `compute_bounds` and `normalise` in `pipeline/scoring/normalise.py`
  ...
  - [ ]* 4.2 Write property test — directional normalisation correctness
  ...
  - [ ]* 4.3 Write property test — normalised features lie in [0, 1]
    - **Property 3: Normalised features lie in [0, 1]** (every normalised feature within the inclusive `[0, 1]` range)
    - **Validates: Requirements 4.4**

  - [ ]* 4.4 Write property test — normalisation bounds come from the eligible population only
...
- [ ] 5. Implement the pure Scoring_Function
  - [ ] 5.1 Implement `score_frame` in `pipeline/scoring/score.py`
  ...
  - [ ] 5.2 Apply the optional confidence discount to score and contributions
  ...
  - [ ]* 5.3 Write property test — weighted-sum scoring correctness
  ...
  - [ ]* 5.4 Write property test — suitability score lies in [0, 1]
  ...
  - [ ]* 5.5 Write property test — contributions reconcile to the score
  ...
  - [ ]* 5.6 Write property test — confidence discount relation
  ...
  - [ ]* 5.7 Write property test — no circular modelling
...
- [ ] 6. Implement ranking
  - [ ] 6.1 Implement `assign_ranks` in `pipeline/scoring/rank.py`
  ...
  - [ ]* 6.2 Write property test — deterministic rank ordering with documented tie-break
...
- [ ] 7. Implement confidence carry-through and excluded-cell nulling
  - [ ] 7.1 Assemble confidence carry-through and excluded-cell nulling in `pipeline/scoring/score.py`
  ...
  - [ ]* 7.2 Write property test — only eligible cells are scored; excluded cells are null and unranked
  ...
  - [ ]* 7.3 Write property test — confidence carried through and two-valued
...
- [ ] 8. Checkpoint — pure core verified
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Implement the output writer
  - [ ] 9.1 Implement `write_scored_table` in `pipeline/scoring/write.py`
  ...
  - [ ]* 9.2 Write property test — cell_id preservation and one row per cell
...
- [ ] 10. Implement the method report and provenance
  - [ ] 10.1 Implement `write_method_report` in `pipeline/scoring/report.py`
  ...
  - [ ] 10.2 Implement `record_provenance` in `pipeline/scoring/report.py`
  ...
  - [ ]* 10.3 Write unit tests for writer, method report, provenance, and naming
...
- [ ] 11. Wire the run() entry point
  - [ ] 11.1 Implement `run(verbose=False, weights_path=None, integrated_path=None, confidence_discount=None) -> dict` in `pipeline/scoring/run.py`
...
- [ ] 12. Register the stage in the orchestrator
  - [ ] 12.1 Register `scoring` in `pipeline/config.py`
  ...
  - [ ] 12.3 Add the scoring subpackage docstring
...
- [ ] 13. Add no-silent-passes validation checks
  - [ ] 13.1 Implement `validate(...)` in `pipeline/scoring/validate.py` and cross-domain checks in `pipeline/validate.py`
  ...
  - [ ]* 13.3 Write unit tests for scoring logic with known inputs and outputs (Requirement 15)
...
- [ ] 15. Full-NSW-grid integration and smoke tests
  ...
  - [ ]* 15.2 Write orchestrator + default-config smoke tests
